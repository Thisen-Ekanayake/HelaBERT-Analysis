"""
writing_style_finetune.py
BERT fine-tuning for Writing Style Classification.
...
DATA_PATH        = os.environ.get("DATA_PATH",        "data/Writing-style-classification/writesty3.csv")

OUT_DIR  = os.environ.get("OUT_DIR",  "output/writing_style")
...
WANDB_PROJECT = os.environ.get("WANDB_PROJECT", "bert-writing-style-finetuning")
...
def train(config: dict, run_name: str, report_to: str = "wandb"):
    ...
    with open(os.path.join(run_out, "run_config.json"), "w") as f:
        json.dump({"run_name": run_name, "task": "writing_style", **config}, f, indent=2)
...
def run_hpo():
    print(f"\n{'='*60}")
    print(f"HPO MODE  |  TASK=writing_style  TRIALS={HPO_TRIALS}")
    print(f"{'='*60}\n")
    ...
    hpo_path = os.path.join(LOG_DIR, "hpo_writing_style_results.json")
...
def run_single():
    ...
    ts       = int(time.time())
    run_name = f"writing_style_lr{LR:.0e}_bs{MICRO_BS}_{ts}"

    print(f"\n{'='*60}")
    print(f"SINGLE RUN  |  TASK=writing_style")
...
if __name__ == "__main__":
    if HPO_MODE:
        ...
        if retrain:
            ...
            ts       = int(time.time())
            run_name = f"writing_style_best_{ts}"
            if USE_WANDB:
                wandb.init(project=WANDB_PROJECT, name=run_name)
