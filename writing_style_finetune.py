"""
BERT Fine-tuning for Sentiment Classification with W&B Logging
...
TRAINING_JOB_NAME = os.getenv("TRAINING_writing_style", "BERT Fine-tuning")
...
# Dataset Path
DATA_PATH = "data/Writing-style-classification/writesty3.csv"  # ← CHANGE THIS to your CSV path

# ==================== LOAD OPTUNA HYPERPARAMETERS ====================
OPTUNA_PARAMS_FILE = "optimal_params/writing_style_finetune.json"
...
LEARNING_RATE      = _optuna_params.get("learning_rate", 4.986345969172954e-05)
WEIGHT_DECAY       = _optuna_params.get("weight_decay",  0.04360386910274888)
WARMUP_RATIO       = _optuna_params.get("warmup_ratio",  0.05201682420690792)
TRAIN_BATCH_SIZE   = int(_optuna_params.get("batch_size", 64))
EVAL_BATCH_SIZE    = TRAIN_BATCH_SIZE
CLASSIFIER_DROPOUT = _optuna_params.get("dropout",       0.1)
...
# Output Directory
OUTPUT_DIR = "HelaBERT_finetuned_sentiment"
...
WANDB_RUN_NAME     = f"bert_lr{LEARNING_RATE}_bs{TRAIN_BATCH_SIZE}_ep{NUM_EPOCHS}_sentiment"
WANDB_ENTITY       = None
WANDB_RUN_ID_FILE  = "wandb_run_id_sentiment.txt"
...
print("=" * 80)
print(f"Training for {NUM_EPOCHS} epochs — {NUM_LABELS} sentiment classes:")
...
# Save label mapping alongside the model for easy inference later
...
# Use original string label names as target names
...
per_class_report = pd.DataFrame({
    ...
    'Sentiment': [id_to_label[i] for i in range(NUM_LABELS)],
...
if USE_WANDB:
    ...
    for metric in ['Precision', 'Recall', 'F1-Score']:
        ...
        table = wandb.Table(data=data, columns=["Sentiment", metric])
        wandb.log({f"per_class_{metric.lower().replace('-', '_')}":
                   wandb.plot.bar(table, "Sentiment", metric, title=f"Per-Sentiment {metric}")})
...
if USE_WANDB and sample_predictions:
    wandb.log({"sample_predictions": wandb.Table(
        ...
        columns=["Text", "True Sentiment", "Predicted Sentiment", "Confidence", "Result"]
    )})
...
print(f"  - predictions.csv          (all validation predictions with sentiment names)")
