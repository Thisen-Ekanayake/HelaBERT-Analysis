...
# ==================== DATASET CLASS ====================
class CrossAttnWritingStyleDataset(Dataset):
    """
    Each sample exposes:
      ...
      comment_ids  [COMMENT_MAX_LENGTH]      — comment text as query
...
def load_fresh_model(bert_model_path, bert_config_file, num_labels):
    ...
    model = CrossAttnWritingStyleModel(
        bert=bert_backbone,
...
class CrossAttnWritingStyleModel(nn.Module):
    """
...
class CrossAttnTrainer(Trainer):
    ...
    def create_scheduler(self, num_training_steps: int, optimizer=None):
        """
        Re-compute num_training_steps from our actual DataLoader length so the
        LR scheduler doesn't receive a bad step count when get_train_dataloader()
        is overridden. Fixes learning_rate=0 at step 1 on AMD ROCm.
        """
        num_training_steps = (
...
for fold_idx, (train_idx, val_idx) in enumerate(skf.split(all_texts, all_labels), start=1):
    ...
    # ── Datasets ──────────────────────────────────────────────────────────────
    train_ds = CrossAttnWritingStyleDataset(
    ...
    val_ds = CrossAttnWritingStyleDataset(
    ...
    training_args = TrainingArguments(
        ...
        fp16=USE_FP16 and torch.cuda.is_available() and not USE_BF16,
        bf16=USE_BF16 and torch.cuda.is_available(),
        adam_epsilon=1e-6,          # AMD ROCm BF16 fix: default 1e-8 underflows in BF16
        optim="adamw_torch",        # AMD ROCm BF16 fix: PyTorch AdamW is more stable on ROCm
        ...
        for si in range(min(5, len(val_ds))):
            ...
            for ci, cw in enumerate(chunk_importance):
                attn_rows.append({
                    ...
                    'text': comment_text,
                    'true_style': true_lbl, 'pred_style': pred_lbl,
