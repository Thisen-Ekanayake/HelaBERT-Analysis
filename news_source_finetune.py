"""
BERT Fine-tuning for News Source Classification — Hyperparameter Grid Search
...
Per-epoch metrics are written to:
    results/news_source_finetune/<run_name>.json
...
NUM_LABELS   = 9       # auto-detected from data; updated if different
MAX_LENGTH   = 64
NUM_EPOCHS   = 20      # fixed
WEIGHT_DECAY = 0.05
...
# Results output dir
SCRIPT_NAME = os.path.splitext(os.path.basename(__file__))[0]   # "news_source_finetune"
...
print("=" * 80)
print("BERT FINE-TUNING — NEWS SOURCE — HYPERPARAMETER GRID SEARCH")
print("=" * 80)
...
class EpochJsonLogger(TrainerCallback):
    ...
    def __init__(self, save_path: str, run_config: dict):
        ...
        self.epochs     = []    # one dict per epoch
        self._pending   = {}    # staging area within current epoch
...
for run_idx, (bs, lr, wr) in enumerate(grid, start=1):
    ...
    try:
        ...
        # Write whatever epochs were completed so far
        epoch_logger.on_train_end(training_args, trainer.state, None)
        if USE_WANDB:
            wandb.finish(exit_code=1)
        continue
