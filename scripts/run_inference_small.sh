#!/usr/bin/env bash
# Launch every inference script in inference/helabert_small, each in its own
...
SCRIPT_DIR="$REPO_ROOT/inference/helabert_small"
LOG_DIR="$REPO_ROOT/logs/inference/small"
mkdir -p "$LOG_DIR"
...
for script_path in "$SCRIPT_DIR"/*.py; do
    ...
    session_name="infer_small_${script_name}"
...
echo
echo "All small-model inference sessions launched. List them with: tmux ls"
