#!/usr/bin/env bash
# Launch every finetune script in finetune/helabert_small, each in its own
...
SCRIPT_DIR="$REPO_ROOT/finetune/helabert_small"
LOG_DIR="$REPO_ROOT/logs/finetune/small"
mkdir -p "$LOG_DIR"
...
for script_path in "$SCRIPT_DIR"/*.py; do
    ...
    session_name="small_${script_name}"
...
echo
echo "All small-model sessions launched. List them with: tmux ls"
