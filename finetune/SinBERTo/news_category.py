"""
Fine-tuning SinBERTo for News Category Classification — 5-Fold Cross Validation
— Balanced training via oversampling + weighted loss —
— Comparison baseline against HelaBERT —

Architecture:
    text → SinBERTo encoder → [CLS] → LayerNorm → Dropout → Linear → num_labels
    (full fine-tuning)
...
# ==================== CONFIGURATION ====================
print("=" * 80)
print("SinBERTo FINE-TUNING — 5-FOLD CV  [NEWS CATEGORY CLASSIFICATION]")
print("=" * 80)
...
OUTPUT_DIR     = "SinBERTo_finetuned_news_category_cv"
...
USE_WANDB      = True
WANDB_PROJECT  = "SinBERTo-news-category-finetuning"
...
class ClassificationModel(nn.Module):
    """
    SinBERTo encoder → [CLS] → LayerNorm → Dropout → Linear → num_labels
...
final_summary_lines.append("\n" + "=" * 80)
final_summary_lines.append(f"5-FOLD CV COMPLETE — SinBERTo / News Category Classification")
final_summary_lines.append("=" * 80)
...
# ==================== SAVE EVAL RESULTS ====================
_eval_results_dir = os.path.join("eval_results", "SinBERTo")
