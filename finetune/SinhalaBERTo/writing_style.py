"""
Fine-tuning SinhalaBERTo for Writing Style Classification — 5-Fold Cross Validation
— Balanced training via oversampling + weighted loss —
— Comparison baseline against HelaBERT —

Architecture:
    text → SinhalaBERTo encoder → [CLS] → LayerNorm → Dropout → Linear → num_labels
    (full fine-tuning)
...
# ==================== CONFIGURATION ====================
print("=" * 80)
print("SinhalaBERTo FINE-TUNING — 5-FOLD CV  [WRITING STYLE CLASSIFICATION]")
print("=" * 80)
...
OUTPUT_DIR     = "SinhalaBERTo_finetuned_writing_style_cv"
...
USE_WANDB      = True
WANDB_PROJECT  = "SinhalaBERTo-writing-style-finetuning"
...
class ClassificationModel(nn.Module):
    """
    SinhalaBERTo encoder → [CLS] → LayerNorm → Dropout → Linear → num_labels
...
final_summary_lines.append("\n" + "=" * 80)
final_summary_lines.append(f"5-FOLD CV COMPLETE — SinhalaBERTo / Writing Style Classification")
final_summary_lines.append("=" * 80)
...
# ==================== SAVE EVAL RESULTS ====================
_eval_results_dir = os.path.join("eval_results", "SinhalaBERTo")
