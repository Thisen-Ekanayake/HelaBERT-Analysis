"""
HelaBERT Fine-tuning for Writing Style Classification
Paper Method: "BERTifying Sinhala - A Comprehensive Analysis of Pre-trained
Language Models for Sinhala Text Classification" (LREC 2022)

Method (Table 3, SinBERT column):
  - Standard fine-tuning: [CLS] token → dropout → linear classifier head
  - Optimizer: AdamW, lr=1e-5, batch=16, epochs=10
  - Evaluation: macro-averaged F1 over 5 randomly-initialized runs, 4:1 train/test split
  - No oversampling, no class weights
  - Truncation is used to fit long documents into the model (paper explicitly states this)

Task: 4-class document classification (long documents)
Classes: News, Academic, Blog, Creative
Dataset: 12,514 samples, avg text length ~181.97 characters (long documents)
Note: Paper reports this as the easiest task (SinBERT-small 95.00 macro-F1)

Architecture:
  BERT encoder → [CLS] → dropout(0.1) → Linear(hidden, 4)
  Input truncated to MAX_SEQ_LENGTH=512 tokens (paper uses truncation for this task)
"""
...
# Writing style has long documents — paper uses TRUNCATION to fit into model.
...
class WritingStyleDataset(Dataset):
    ...
    Paper: "Since the dataset contains long text, we use truncation to fit them into the models."
    """
...
print(f"Paper baseline (SinBERT-small): 95.00")
