"""
HelaBERT Inference for Writing Style Classification
Loads saved checkpoints from HelaBERT_paper_writing_style/run_N/ and evaluates
on the same test split used during training (reproduces seed-based 4:1 splits).
Long documents are truncated to 512 tokens, matching the training pre-processing.

Usage (from /ml/HelaBERT_Analysis):
    python finetune/helabert_small/infer_writing_style.py
    python finetune/helabert_small/infer_writing_style.py --run 1
    python finetune/helabert_small/infer_writing_style.py --all-runs
"""
...
MAX_SEQ_LENGTH = 512    # same truncation as training
BATCH_SIZE     = 16     # long docs — keep batch small to avoid OOM
DROPOUT        = 0.1
NUM_LABELS     = 4
TEST_SIZE      = 0.2
...
class WritingStyleDataset(Dataset):
    """Long documents truncated to MAX_SEQ_LENGTH (paper method)."""
...
def load_model_from_checkpoint(ckpt_dir, num_labels, hidden_size):
    ...
    model = WritingStyleModel(backbone, hidden_size, num_labels, DROPOUT)
...
def evaluate_run(run_idx, all_texts, all_labels_enc, le, num_labels, device):
    ...
    print(f"Test samples: {len(test_texts):,}  (truncated to {MAX_SEQ_LENGTH} tokens)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inference for HelaBERT Writing Style")
    ...
    print("=" * 80)
    print("HelaBERT INFERENCE — WRITING STYLE")
    print("=" * 80)
    ...
    if num_labels_actual != NUM_LABELS:
        print(f"WARNING: Expected {NUM_LABELS} classes, found {num_labels_actual}")
