"""
HelaBERT Fine-tuning for Sentiment Analysis — WITH CO-ATTENTION (FIXED)
...
FIXES over previous co-attention version:
  1. NaN-safe softmax: clamp affinity so no row is all -inf before softmax.
  2. Correct beta direction: token_seq attends to [CLS] by treating [CLS] as
     the key/value and each token's affinity score as its query weight.
     The previous transpose(0,1) was swapping batch and time dimensions — wrong.
  3. LayerNorm on attended outputs before concat for training stability.

All hyperparameters (lr, batch, epochs, dropout, etc.) are UNCHANGED.
"""
