"""
HelaBERT Inference for News Source Classification
Loads saved checkpoints from HelaBERT_paper_news_source/run_N/ and evaluates
on the same test split used during training (reproduces seed-based 4:1 splits).

Usage (from /ml/HelaBERT_Analysis):
    python finetune/helabert_small/infer_news_source.py
    python finetune/helabert_small/infer_news_source.py --run 1
    python finetune/helabert_small/infer_news_source.py --all-runs
"""
...
def load_model_from_checkpoint(ckpt_dir, num_labels, hidden_size):
    ...
    model = NewsSourceModel(backbone, hidden_size, num_labels, DROPOUT)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inference for HelaBERT News Source")
    ...
    print("=" * 80)
    print("HelaBERT INFERENCE — NEWS SOURCE")
    print("=" * 80)
