"""
HelaBERT Inference for News Category Classification
Loads saved checkpoints from HelaBERT_paper_news_category/run_N/ and evaluates
on the same test split used during training (reproduces seed-based 4:1 splits).

Usage (from /ml/HelaBERT_Analysis):
    python finetune/helabert_small/infer_news_category.py
    python finetune/helabert_small/infer_news_category.py --run 1
    python finetune/helabert_small/infer_news_category.py --all-runs
"""
...
OUTPUT_DIR       = "HelaBERT_paper_news_category"
...
def find_checkpoint_dir(run_dir):
    ...
    if ckpts:
        return ckpts[-1]   # latest / only checkpoint
...
def load_model_from_checkpoint(ckpt_dir, num_labels, hidden_size):
    ...
    model = NewsCategoryModel(backbone, hidden_size, num_labels, DROPOUT)
...
def evaluate_run(run_idx, all_texts, all_labels_enc, le, device):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inference for HelaBERT News Category")
    ...
    print("=" * 80)
    print("HelaBERT INFERENCE — NEWS CATEGORY")
    print("=" * 80)
    ...
    print(f"Loaded {len(df):,} samples, classes: {list(le.classes_)}")
    ...
    for run_idx in runs_to_eval:
        macro_f1 = evaluate_run(run_idx, all_texts, all_labels_enc, le, device)
    ...
    if len(run_f1s) > 1:
        ...
        print(f"Paper baseline (SinBERT-small): 84.75")
