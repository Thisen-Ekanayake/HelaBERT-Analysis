...
def _build_success_email(summary: dict, eval_results: dict, wandb_url: str = None,
...
OPTUNA_PARAMS_FILE = "optimal_params/news_category_finetune.json"
...
LEARNING_RATE              = _optuna_params.get("learning_rate",  3.337685579192411e-05)
WEIGHT_DECAY               = _optuna_params.get("weight_decay",   0.060560258574995335)
WARMUP_RATIO               = _optuna_params.get("warmup_ratio",   0.1817068676063539)
...
class EpochLoggerCallback(TrainerCallback):
    ...
    def __init__(self):
        self.epoch_logs: list = []          # one dict per epoch
        self._pending_train_loss: float = None
        self._step_losses: list = []        # accumulate step losses within an epoch

    def on_log(self, args, state, control, logs=None, **kwargs):
        """Called at every logging step — collect step-level training loss."""
        if logs and "loss" in logs:
            self._step_losses.append(logs["loss"])

    def on_epoch_end(self, args, state, control, **kwargs):
        """Called at the end of each training epoch — compute mean train loss."""
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        """Called right after evaluation — merge eval metrics with latest train loss."""
        if metrics is None:
            return
        ...
        for key, value in metrics.items():
            clean_key = key.replace("eval_", "")           # e.g. eval_loss → loss
            record[f"eval_{clean_key}"] = value
...
for rec in epoch_logger.epoch_logs:
    print(
        ...
        f"{rec.get('eval_eval_loss', rec.get('eval_loss', float('nan'))):>9.4f} | "
