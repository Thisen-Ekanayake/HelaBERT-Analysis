#!/usr/bin/env bash
# Launch a fixed subset of finetune scripts, each in its own detached tmux
# session (6 sessions total). Run from repo root.
set -euo pipefail
...
LOG_DIR="$REPO_ROOT/logs/finetune/selected"
mkdir -p "$LOG_DIR"
...
SCRIPTS=(
    "finetune/helabert_large/news_category.py"
    "finetune/helabert_large/news_category_co_attn.py"
    "finetune/helabert_small/news_category.py"
    "finetune/helabert_small/news_category_co_attn.py"
    "finetune/helabert_small/sentiment.py"
...
echo
echo "Selected finetune sessions launched. List them with: tmux ls"
