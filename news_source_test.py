"""
Evaluation script for the fine-tuned BERT News Source Classification model.

Loads the best model saved by news_source_finetune.py and evaluates it
...
Usage:
    python evaluate_news_source.py
    python evaluate_news_source.py --test_path path/to/test.csv --model_dir path/to/best_model
"""
...
DEFAULT_MODEL_DIR       = "HelaBERT_finetuned_news_source_cv/best_model"
DEFAULT_TOKENIZER_MODEL = "tokenizer/unigram_32000_0.9995.model"
DEFAULT_TEST_PATH       = "data/Sinhala-News-Source-classification/test/news_source_test.csv"
DEFAULT_OUTPUT_DIR      = "eval_results_news_source"

NUM_LABELS  = 9
MAX_LENGTH  = 32       # matches training — source classification uses shorter sequences
BATCH_SIZE  = 16
NUM_WORKERS = 2
...
class SentencePieceDataset(Dataset):
    def __init__(self, texts, labels, sp_processor, max_length=32):
        self.texts      = texts
        self.labels     = labels
...
# ==================== MAIN ====================
def main():
    parser = argparse.ArgumentParser(description="Evaluate saved BERT news-source model")
    ...
    print("=" * 70)
    print("BERT NEWS SOURCE — EVALUATION")
    print("=" * 70)
    ...
    target_names = [f"Source_{i}" for i in range(num_labels)]
