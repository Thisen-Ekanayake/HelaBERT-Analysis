...
SOURCE_NAMES = {
    0: "Source_0", 1: "Source_1", 2: "Source_2",
    3: "Source_3", 4: "Source_4", 5: "Source_5",
    6: "Source_6", 7: "Source_7", 8: "Source_8",
}
...
EVAL_BATCH_SIZE              = 8
LEARNING_RATE                = 2e-5
NUM_EPOCHS                   = 12      # early stopping decides actual epoch count
WARMUP_RATIO                 = 0.05
WEIGHT_DECAY                 = 0.05
...
# Output
OUTPUT_DIR     = "HelaBERT_finetuned_news_source_baseline_cv"
...
# ==================== DATASET CLASS ====================
class BaselineNewsSourceDataset(Dataset):
    """
    Each sample exposes:
...
# ==================== MODEL ====================
class BaselineNewsSourceModel(nn.Module):
    """
...
# Sync NUM_LABELS and SOURCE_NAMES with the actual data
...
if actual_num_labels != NUM_LABELS:
    ...
    SOURCE_NAMES = {i: f"Source_{i}" for i in range(NUM_LABELS)}

print(f"Loaded {len(df)} samples, {NUM_LABELS} classes")
print("\nFull dataset label distribution:")
for lbl, cnt in df['label'].value_counts().sort_index().items():
    print(f"  Label {lbl} ({SOURCE_NAMES.get(lbl, '?'):15s}): {cnt:6d} ({100 * cnt / len(df):.1f}%)")
...
for fold_idx, (train_idx, val_idx) in enumerate(skf.split(all_texts, all_labels), start=1):
    ...
    # ==================== Datasets ====================
    train_ds = BaselineNewsSourceDataset(
        fold_train_texts, fold_train_labels, sp,
        comment_max_length=COMMENT_MAX_LENGTH,
    )
    val_ds = BaselineNewsSourceDataset(
    ...
    target_names = [SOURCE_NAMES.get(i, f"Source_{i}") for i in range(NUM_LABELS)]
    ...
    pd.DataFrame({
        'text':               fold_val_texts,
        'true_label':         y_true,
        'true_source':        [SOURCE_NAMES.get(l, str(l)) for l in y_true],
        'predicted_label':    y_pred,
        'predicted_source':   [SOURCE_NAMES.get(l, str(l)) for l in y_pred],
    ...
    per_class_df = pd.DataFrame({
        ...
        'Source':    [SOURCE_NAMES.get(i, f"Source_{i}") for i in range(NUM_LABELS)],
        'Precision': prec_pc,
        'Recall':    rec_pc,
        'F1-Score':  f1_pc,
        'Support':   sup_pc,
    ...
    pd.DataFrame(
        cm,
        index=[f"True_{SOURCE_NAMES.get(i, i)}"   for i in range(NUM_LABELS)],
        columns=[f"Pred_{SOURCE_NAMES.get(i, i)}" for i in range(NUM_LABELS)],
    ...
    if USE_WANDB:
        ...
        try:
            ...
            fig = ff.create_annotated_heatmap(
                z=cm,
                x=[f"Pred_{SOURCE_NAMES.get(i, i)}" for i in range(NUM_LABELS)],
                y=[f"True_{SOURCE_NAMES.get(i, i)}" for i in range(NUM_LABELS)],
                colorscale='Blues',
                showscale=True,
            )
            fig.update_layout(
