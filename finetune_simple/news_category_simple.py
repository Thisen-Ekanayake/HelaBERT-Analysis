...
EVAL_BATCH_SIZE              = 8
LEARNING_RATE                = 2e-5
NUM_EPOCHS                   = 6      # early stopping decides actual epoch count
WARMUP_RATIO                 = 0.05
WEIGHT_DECAY                 = 0.05
...
# Output
OUTPUT_DIR     = "HelaBERT_finetuned_news_category_baseline_cv"
...
# ==================== DATASET CLASS ====================
class BaselineNewsCategoryDataset(Dataset):
    """
    Each sample exposes:
...
# ==================== MODEL ====================
class BaselineNewsCategoryModel(nn.Module):
    """
...
def load_fresh_model(bert_model_path, bert_config_file, num_labels):
    ...
    model = BaselineNewsCategoryModel(
        bert=bert_backbone,
...
for fold_idx, (train_idx, val_idx) in enumerate(skf.split(all_texts, all_labels), start=1):
    ...
    # ==================== Datasets ====================
    train_ds = BaselineNewsCategoryDataset(
        fold_train_texts, fold_train_labels, sp,
        comment_max_length=COMMENT_MAX_LENGTH,
    )
    val_ds = BaselineNewsCategoryDataset(
