...
TEST_DATA_PATH = "data/Sinhala-News-Source-classification/test/news_source_test.csv"  # e.g., "data/test_news_source.csv"
BEST_MODEL_PATH = "utils/finetuned_models/HelaBERT_finetuned_news_source_baseline_cv/best_model"    # e.g., "HelaBERT_finetuned_news_source_baseline_cv/best_model"
...
print("=" * 80)
print("NEWS SOURCE CLASSIFICATION - TEST EVALUATION")
print("=" * 80)
...
class SourceClassifier(nn.Module):
    """BERT [CLS] classifier for news source"""
...
# Extract texts and labels (comment column)
test_texts = df["comment"].values
...
test_dataset = SourceTestDataset(test_texts, test_labels, sp, COMMENT_MAX_LENGTH)
...
# Create classifier
model = SourceClassifier(bert_model, NUM_LABELS)
...
per_class_metrics = pd.DataFrame(
    ...
    columns=[SOURCE_NAMES.get(i, f"Source_{i}") for i in range(NUM_LABELS)],
).T
